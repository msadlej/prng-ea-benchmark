...
// #include <WINDOWS.H>  // Remove this line for Linux
#include <malloc.h>
#include <math.h>
#include <stdio.h>
...
void cf02_s(double *, double *, int, double *, double *,
            int); /* Composition Function 2 for shift case*/
void cf04_s(double *, double *, int, double *, double *,
            int); /* Composition Function 4 for shift case*/
void cf05_s(double *, double *, int, double *, double *,
...
void cec21_basic_func(double *x, double *f, int nx, int mx, int func_num) {
    ...
    if (ini_flag == 0) {
        ...
        if (func_num >= 5 && func_num <= 7) {
            sprintf(FileName, "input_data/shuffle_data_%d_D%d.txt", func_num,
                    nx);
            fpt = fopen(FileName, "r");
            if (fpt == NULL) {
                printf("\n Error: Cannot open shuffle_data_%d_D%d.txt for "
                       "reading \n",
                       func_num, nx);
            }
    ...
    for (i = 0; i < mx; i++) {
        ...
        case 10:
            ...
            break;
        default:
            printf("\nError: There are only 10 test functions in this test "
                   "suite!\n");
            break;
        }
    }
                     ...
                     int s_flag, int r_flag) /* Griewank-Rosenbrock  */
{
    ...
    sr_func(x, z, nx, Os, Mr, 5.0 / 100.0, s_flag,
            r_flag); /* shift and rotate */
