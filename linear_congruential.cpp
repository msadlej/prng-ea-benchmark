...
std::vector<long double> getGaussianDistribution(unsigned long n_samples,
                                                 long double mean = 0.0,
                                                 long double stdev = 1.0) {
    std::random_device rd;
    std::minstd_rand engine(rd());
    std::normal_distribution<long double> dist(mean, stdev);
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("data/decimal/linear_congruential.csv");
    ...
    csv_file.close();
    std::cout << "Numbers written to data/decimal/linear_congruential.csv"
              << std::endl;
