...
Fitness LSHADE::run() {
    ...
    Individual bsf_solution =
        (variable *)malloc(sizeof(variable) * problem_size);
    Fitness bsf_fitness;
    int nfes = 0;
    ...
    while (nfes < max_num_evaluations) {
        ...
        for (int target = 0; target < pop_size; target++) {
            // In each generation, CR_i and F_i used by each individual x_i are
            // generated by first selecting an index r_i randomly from [1, H]
            ...
            // p-best individual is randomly selected from the top pop_size *
            // p_i members
            p_best_ind = sorted_array[rand() % p_num];
            operateCurrentToPBest1BinWithArchive(
                pop, &children[target][0], target, p_best_ind, pop_sf[target],
                pop_cr[target], archive, arc_ind_count);
        }
        ...
        /////////////////////////////////////////////////////////////////////////
        // update the bsf-solution and check the current number of fitness
        // evaluations
        //  if the current number of fitness evaluations over the max number of
        //  fitness evaluations, the search is terminated So, this program is
        //  unconcerned about L-SHADE algorithm directly
        for (int i = 0; i < pop_size; i++) {
            nfes++;

            // following the rules of CEC 2014 real parameter competition,
            // if the gap between the error values of the best solution found
            // and the optimal solution was 10^{−8} or smaller, the error was
            // treated as 0
        ...
        for (int i = 0; i < pop_size; i++) {
            if (children_fitness[i] == fitness[i]) {
                ...
                // parent vectors x_i which were worse than the trial vectors
                // u_i are preserved
                if (arc_size > 1) {
                    ...
                    // Whenever the size of the archive exceeds, randomly
                    // selected elements are deleted to make space for the newly
                    // inserted elements
                    else {
        ...
        // if numeber of successful parameters > 0, historical memories are
        // updated
        ...
        plan_pop_size = round(
            (((min_pop_size - max_pop_size) / (double)max_num_evaluations) *
             nfes) +
            max_pop_size);
...
void LSHADE::operateCurrentToPBest1BinWithArchive(
        ...
        for (int i = 0; i < problem_size; i++) {
            if ((randDouble() < cross_rate) || (i == random_variable)) {
                child[i] = pop[target][i] +
                           scaling_factor *
                               (pop[p_best_individual][i] - pop[target][i]) +
    ...
    // If the mutant vector violates bounds, the bound handling method is
    // applied
    modifySolutionWithParentMedium(child, pop[target]);
}

void LSHADE::reducePopulationWithSort(vector<Individual> &pop,
