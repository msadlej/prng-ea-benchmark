...
int fpfuncdef_full_impl(const CMAParameters<TGenoPheno> &cmaparams,
    ...
    if (cmasols.get_best_seen_candidate().get_x_size())
        fplotstream
            << cmasols.get_best_seen_candidate().get_x_dvec().transpose()
            << sep; // xbest
    else
...
CMAStrategy<TCovarianceUpdate, TGenoPheno>::CMAStrategy(
    ...
    if (!eostrat<TGenoPheno>::_parameters._fplot.empty()) {
        _fplotstream =
            new std::ofstream(eostrat<TGenoPheno>::_parameters._fplot);
...
dMat CMAStrategy<TCovarianceUpdate, TGenoPheno>::ask() {
    ...
    if (!eostrat<TGenoPheno>::_parameters._sep &&
        !eostrat<TGenoPheno>::_parameters._vd)
        pop =
            _esolver.samples(eostrat<TGenoPheno>::_parameters._lambda,
    ...
    if (eostrat<TGenoPheno>::_parameters._with_gradient) {
        dVec grad_at_mean = eostrat<TGenoPheno>::gradf(
            eostrat<TGenoPheno>::_parameters._gp.pheno(
                eostrat<TGenoPheno>::_solutions._xmean));
        dVec gradgp_at_mean = eostrat<TGenoPheno>::gradgp(
            eostrat<TGenoPheno>::_solutions
                ._xmean); // for geno / pheno transform.
    ...
    if (eostrat<TGenoPheno>::_parameters._tpa == 2 &&
        ...
        if (!eostrat<TGenoPheno>::_parameters._sep &&
            ...
            mean_shift_norm =
                (_esolver._eigenSolver.eigenvalues()
                     ...
                     .cwiseInverse()
                     .cwiseProduct(
                         _esolver._eigenSolver.eigenvectors().transpose() *
                         mean_shift))
                    .norm() /
                eostrat<TGenoPheno>::_solutions._sigma;
        else
        ...
        // << std::sqrt(std::sqrt(eostrat<TGenoPheno>::_parameters._dim)) <<
        // std::endl;
...
bool CMAStrategy<TCovarianceUpdate, TGenoPheno>::stop() {
    if (eostrat<TGenoPheno>::_solutions._run_status <
        0) // an error occured, most likely out of memory at cov matrix
           // creation.
        return true;
...
int CMAStrategy<TCovarianceUpdate, TGenoPheno>::optimize(
    ...
    while (!stop()) {
        ...
        evalf(candidates,
              eostrat<TGenoPheno>::_parameters._gp.pheno(candidates));
        tellf();
                ...
                3) // elitist
        {
            LOG_IF(INFO, !eostrat<TGenoPheno>::_parameters._quiet)
                ...
                << eostrat<TGenoPheno>::_solutions._best_seen_candidate
                       .get_fvalue()
                << " / biter="
                << eostrat<TGenoPheno>::_solutions._best_seen_iter << std::endl;
            ...
            eostrat<TGenoPheno>::_solutions._nevals =
                eostrat<TGenoPheno>::_nevals;
            eostrat<TGenoPheno>::_niter = 0;
            optimize();
        }
    }
