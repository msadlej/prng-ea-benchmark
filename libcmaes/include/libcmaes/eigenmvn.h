...
template <typename Scalar> class EigenMultivariateNormal {
  ...
  public:
    ...
    void setCovar(const Matrix<Scalar, Dynamic, Dynamic> &covar) {
        ...
        if (_use_cholesky) {
            Eigen::LLT<Eigen::Matrix<Scalar, Dynamic, Dynamic>> cholSolver(
                _covar);
            ...
            if (cholSolver.info() == Eigen::Success) {
                ...
                throw std::runtime_error("Failed computing the Cholesky "
                                         "decomposition. Use solver instead");
            }
        } else {
            _eigenSolver =
                SelfAdjointEigenSolver<Matrix<Scalar, Dynamic, Dynamic>>(
    ...
    Matrix<Scalar, Dynamic, -1> samples_ind(int nn, double factor) {
        dMat pop = (Matrix<Scalar, Dynamic, -1>::NullaryExpr(_covar.rows(), nn,
                                                             randN)) *
                   factor;
